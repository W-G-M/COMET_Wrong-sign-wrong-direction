...
void Analyzer(TString inFileName, TString outFileName) {
  ...
  // h_cutflow->GetXaxis()->SetBinLabel(2,"Total Positron");
  ...
  TH1D* posTot = new TH1D("posTot", "posTot", 200, 0., 800.);
  ...
  for (size_t i=0; i<nEntries; ++i)
  {
    ...
    {      
      ...
      // IHandle<IMCHit> track_positron0(nullptr);
      int count = 0;
      for (auto& trackMap: *g4Tracks)
      {
        ...
        if(hasPos)
        {        
          // posTot->Fill(track_positron0->GetInitialMomentum().E() / unit::MeV, 1);
          // h_cutflow->Fill("Total Positron", 1);
          cout << "\n" << endl;
          std :: cout <<"particla_name"<<": "<<thisTrack_partName<<", "<<"process name: "<<thisTrack_procName<<endl;          
          std :: cout << "volume name: " << track_positron0->GetTrajectoryPoints().front().GetVolumeName() << " | " << track_positron0->GetTrajectoryPoints().back().GetVolumeName() << std::endl;
          ...
          // << "|" << track_positron0->GetTrajectoryPoints().back().GetVolumeName() << std::endl;
          // count = 0;
          ...
          // if(TString(volumeNameB).Contains("CTH") && TString(volumeNameF).Contains("CTH"))
          // {
          //   pos->Fill(track_positron0->GetInitialMomentum().E() / unit::MeV, 1);
          //   h_cutflow->Fill("Positron", 1);
          // }
          // count = count + 1;
          // cout << "CTH pos counts: " << count << endl;
        }
        //
        // count=0
        // {
        ...
        // cout<<"particla_name"<<": "<<thisTrack_partName<<", "<<"process name: "<<thisTrack_procName<<", "<<"bool: "<<hasPos<<endl;
      ...
      //       std::cout << track_positron0->GetTrajectoryPoints().front().GetVolumeName() << "|" << track_positron0->GetTrajectoryPoints().back().GetVolumeName() << std::endl;
      ...
      //       // cout << track_gamma0->GetTrajectoryPoints().front().GetVolumeName() << "|" << track_gamma0->GetTrajectoryPoints().back().GetVolumeName() << endl;
      //       // cout << track_gamma1->GetTrajectoryPoints().front().GetVolumeName() << "|" << track_gamma1->GetTrajectoryPoints().back().GetVolumeName() << endl;
      //     }
      //     //
      //     //
      //     //
